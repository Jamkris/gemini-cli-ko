# 미리보기 릴리스: 릴리스 v0.28.0-preview.0

릴리스 날짜: 2026년 2월 3일

미리보기 릴리스에는 최신의 새로운 실험적 기능이 포함되어 있습니다.
이 릴리스는 [최신 주간 릴리스](/docs/changelogs/latest)만큼 안정적이지 않을 수 있습니다.

미리보기 릴리스를 설치하려면 다음을 실행하세요:
...
- **향상된 훅 관리:** 훅 활성화/비활성화 기능이 이제 기술(skills)과 일치하며 향상된 자동 완성을 제공합니다.
- **확장 프로그램용 사용자 지정 테마:** 이제 확장에서 사용자 지정 테마를 지원하여 개인화 기능을 강화할 수 있습니다.
- **사용자 신원 표시:** 시작 시와 `stats` 명령에서 사용자 신원 정보(인증, 이메일, 등급)가 표시됩니다.
- **계획 모드 개선 사항:** 계획 모드(Plan Mode)가 일반적인 `Checklist` 구성 요소와 리팩터링된 `Todo`로 개선되었습니다.
- **백그라운드 셸 명령:** 백그라운드 셸 명령 구 현이 포함되었습니다.
