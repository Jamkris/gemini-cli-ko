...
Gemini CLI에는 Nightly, Preview, Stable(안정)이라는 세 가지 주요 릴리스 채널이 있습니다.
대부분의 사용자에게 안정 릴리스를 권장합니다.

이 페이지에서는 현재 릴리스에 관한 정보와 각 릴리스의 공지 사항을 확인할 수 있습니다.

전체 변경 로그는 GitHub의
[Releases - google-gemini/gemini-cli](https://github.com/google-gemini/gemini-cli/releases)를
참조하세요.
...
| 릴리스 채널                         | 비고                                                |
| :---------------------------------- | :-------------------------------------------------- |
| Nightly                             | 가장 최근 변경 사항이 포함된 Nightly 릴리스입니다.  |
| [Preview](/docs/changelogs/preview) | 조기 피드백을 위한 실험적 기능이 준비되어 있습니다. |
| [Stable](/docs/changelogs/latest)   | 안정적이며 일반적인 사용에 권장됩니다.              |

## 공지 사항: v0.27.0 - 2026-02-03

- **이벤트 기반 아키텍처:** CLI는 이제 도구 실행에 새로운 이벤트 기반 스케줄러를 사용하여
  더 반응성이 뛰어나고 성능이 우수한 경험을 제공합니다
  ([#17078](https://github.com/google-gemini/gemini-cli/pull/17078) by
  @abhipatel12).
- **향상된 사용자 경험:** 이번 릴리스에는 대기 중인 도구 확인과 더 원활한 워크플로를 위한
  확장 가능한 큰 텍스트 붙여넣기가 포함되어 있습니다.
- **새로운 `/rewind` 명령:** 새로운 `/rewind` 명령으로 세션 기록을 쉽게 탐색하세요
  ([#15720](https://github.com/google-gemini/gemini-cli/pull/15720) by
  @Adib234).
- **Linux 클립보드 지원:** 이제 Wayland 및 X11이 있는 Linux에서 이미지를 붙여넣을 수 있습니다
  ([#17144](https://github.com/google-gemini/gemini-cli/pull/17144) by
  @devr0306).

## 공지 사항: v0.26.0 - 2026-01-27

- **에이전트 및 기술:** 새로운 `skill-creator` 기술을 도입하고
  ([#16394](https://github.com/google-gemini/gemini-cli/pull/16394) by
  @NTaylorMullen), 에이전트 기술을 기본적으로 활성화했으며, 작업 라우팅을 개선하기 위해
  일반 에이전트를 추가했습니다
  ([#16638](https://github.com/google-gemini/gemini-cli/pull/16638) by
  @joshualitt).
- **UI/UX 개선:** 대화 기록을 통해 "되감기(Rewind)" 할 수 있으며
  ([#15717](https://github.com/google-gemini/gemini-cli/pull/15717) by @Adib234)
  디버깅을 위해 새로운 `/introspect` 명령을 사용할 수 있습니다.
- **코어 및 스케줄러 리팩터링:** 성능과 안정성을 향상시키기 위해 코어 스케줄러가
  상당히 리팩터링되었으며
  ([#16895](https://github.com/google-gemini/gemini-cli/pull/16895) by
  @abhipatel12), 수많은 성능 및 안정성 수정 사항이 포함되었습니다.

## 공지 사항: v0.25.0 - 2026-01-20

- **기술 및 에이전트 개선:** `activate_skill` 도구를 개선하고, 새로운 `pr-creator` 기술을
  추가하고 ([#16232](https://github.com/google-gemini/gemini-cli/pull/16232) by
  [@NTaylorMullen](https://github.com/NTaylorMullen)), 기술을 기본적으로 활성화하고,
  `cli_help` 에이전트를 개선하고
  ([#16100](https://github.com/google-gemini/gemini-cli/pull/16100) by
  [@scidomino](https://github.com/scidomino)), 새로운 `/agents refresh` 명령을 추가했습니다
  ([#16204](https://github.com/google-gemini/gemini-cli/pull/16204) by
  [@joshualitt](https://github.com/joshualitt)).
- **UI/UX 개선:** 기술에 대한 더 투명한 피드백
  ([#15954](https://github.com/google-gemini/gemini-cli/pull/15954) by
  [@NTaylorMullen](https://github.com/NTaylorMullen)), Tab 키를 사용하여 셸과 입력 간에
  포커스를 전환하는 기능
  ([#14332](https://github.com/google-gemini/gemini-cli/pull/14332) by
  [@jacob314](https://github.com/jacob314)), 그리고 동적 터미널 탭 제목
  ([#16378](https://github.com/google-gemini/gemini-cli/pull/16378) by
  [@NTaylorMullen](https://github.com/NTaylorMullen))을 확인할 수 있습니다.
- **핵심 기능 및 성능:** 이번 릴리스에는 내장 에이전트 기술 지원
  ([#16045](https://github.com/google-gemini/gemini-cli/pull/16045) by
  [@NTaylorMullen](https://github.com/NTaylorMullen)), 개선된 Gemini 3 시스템 지침
  ([#16139](https://github.com/google-gemini/gemini-cli/pull/16139) by
  [@NTaylorMullen](https://github.com/NTaylorMullen)), 성능 향상을 위한 무시 인스턴스 캐싱
  ([#16185](https://github.com/google-gemini/gemini-cli/pull/16185) by
  [@EricRahm](https://github.com/EricRahm)), 향상된 재시도 메커니즘
  ([#16489](https://github.com/google-gemini/gemini-cli/pull/16489) by
  [@sehoon38](https://github.com/sehoon38))이 포함되어 있습니다.
- **버그 수정 및 안정성:** CLI, 코어 및 워크플로 전반에 걸쳐 수많은 버그를 수정했으며,
  서브 에이전트 위임, 유니코드 문자 충돌 및 고정 헤더 회귀와 관련된 문제를 해결했습니다.

## 공지 사항: v0.24.0 - 2026-01-14

- **에이전트 기술:** 에이전트 기술이 크게 발전했습니다. 여기에는 시작하는 데 도움이 되는
  초기 문서 및 튜토리얼과 함께 원격 에이전트에 대한 향상된 지원이 포함되어 있어
  Gemini CLI 내에서 더욱 분산되고 강력한 자동화가 가능합니다.
...
- **향상된 UI/UX:** 사용자 인터페이스가 훅 실행에 대한 시각적 표시기, 더 세련된 설정 표시,
  Tab 키를 사용하여 셸과 입력 간에 쉽게 포커스를 전환하는 기능을 포함하여
  여러 업데이트를 받았습니다.
  and input areas.
